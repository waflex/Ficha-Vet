...
export const crearFicha = async (req,res)=>{
  if(req.body.rut){//verficar checkbox
    console.log(req.body)
  }
  if(req.body.rutChip){//verficar checkbox

  }
  try {
    const {celular,comentarios,correo,direccion,especie,nombreMascota,nombreTutor,raza,rut,rutChip} =req.body
    const newTutor = await CrearTutor(rut,nombreTutor,correo,celular,direccion);
    const newPet =  await CrearMascota(rutChip,nombreMascota,especie,raza,Antencedentes);
...
async function CrearMascota(Rut_Ficha_Masc,Nombre,Especie,Raza,Rut_Tutor,Antencedentes){
    Mascota({
        Rut_Ficha_Masc,
        Nombre,
        Especie,
